...
| Object storage | **SeaweedFS 3.73** | `deploy/docker-compose.oss.yml` | Blob/artifact store in the OSS compose stack. Chosen and stood up, but **nothing in `engine/` imports it yet** — the forensic/cold-tier path is unbuilt. |
...
| Bundle delivery | **`go:embed`** (engine) / **nginx** (control plane) | `engine/internal/api/web_embed.go` (`//go:embed all:web`) | For the single-tenant engine the React build is compiled *into* the Go binary — zero Node runtime, no external web server. The multi-tenant control plane serves the same bundle from nginx, which is already there for TLS. |
