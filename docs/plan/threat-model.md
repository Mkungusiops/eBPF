...
([`docs/operations/enforcement-traps.md`](../operations/enforcement-traps.md)).
Two compounding traps from the live box:
...
2. **Tetragon's own TracingPolicies enforce independently** of the engine's mode
   (trap 2 in the same document): flipping the *engine* to detect-only does
   **not** disable Tetragon `Sigkill` policies (e.g. `override-credential-read`,
   `sever-pipe-to-shell`). This already **broke `apt`** on the live box. There
...
| ID | Threat (STRIDE) | Adversary | Mitigation | Phase |
|----|-----------------|-----------|------------|:---:|
| EN-1 | **Self-lockout** — enforcement kills recovery paths (`sudo`, `sshd`, `systemd`, package mgmt) (D) | operator error, A5 | **Detect-only default** everywhere; central **protected-process / system-critical allow-lists** (`choke.DefaultSystemCriticalBinaries()` bypasses score-driven enforce) covering sudo/ssh/init/pkg-mgmt; mode is explicit and its persistence is well-defined. **Observed twice — see EN-1a/EN-1b.** | 2 |
| EN-1a | **Allow-lists rot against upstream changes** — OpenSSH ≥9.8 split the daemon, so the process that reads credentials is `sshd-session`/`sshd-auth`, not `sshd` (D) | upstream change | Allow-listing bare `sshd` silently became a **remote lockout** on Ubuntu 24.10+/Debian 13/RHEL 10: pubkey auth died the instant the key was offered. Now lists every layout (`/usr/lib/openssh/`, `/usr/libexec/openssh/`) **plus `login`** so the console recovery path can never be killed either, with a regression test pinning it. Recovery, if it happens again: `sshd` is up seconds after boot but Tetragon only kills once it has attached — reboot and race that window with a retry loop that removes the container. | 1 |
| EN-1b | **`matchBinaries` cannot match interpreted scripts** — the executable is the interpreter (D) | design limit | For `#!/usr/bin/perl` scripts the executable is `/usr/bin/perl`, so listing the *script's* path in a `NotIn` allow-list has **no effect**. `debconf/frontend` and `dpkg-reconfigure` are scripts, so package maintainer scripts were SIGKILLed mid-configure — packages stuck `iF`/`iU` (including `libpam-runtime`), all `apt` blocked, driven silently by `unattended-upgrades` on a timer. **Not fixable by adding paths**: correctness would require allow-listing `/usr/bin/perl` and `/bin/sh`, i.e. most attacker tooling. Resolution: that policy **detects**, and enforcement for the signal lives in the engine's choke gateway, which matches the process it actually scored, honours the critical-binary list, and offers a reversible ladder with an audit row. | 1 |
| EN-1c | **`followChildren` is silently ignored** — the obvious repair for EN-1b does not work (D) | upstream limit | The natural fix for EN-1b is ancestor scoping: exempt anything descended from `dpkg`/`apt` rather than naming interpreters. **Measured on Tetragon v1.6.1 and it does not work.** With `NotIn` + `followChildren: true`, a child of a listed binary was still SIGKILLed; with `In` + `followChildren: true`, a child of a listed binary was **not** matched. Both are consistent with the flag being a no-op on `security_file_permission`, and the policy loads reporting `added` with **no warning** — a silent no-op is the worst failure mode, since it reads as a working control. Note also that ancestor scoping is structurally wrong here even if it worked: the allow-list contains `systemd` (PID 1 — every process is its descendant, so the policy would match nothing) and `sshd` (exempting every interactive session). Kernel-level enforcement of this signal has **no correct formulation**; it belongs in the engine. | 1 |
| EN-1d | **Enforcement that silently does nothing** — a policy shipped as `enforce/` that never fires (D,R) | design error | `sever-pipe-to-shell.yaml` shipped `action: Sigkill` for the `curl\|sh` pattern and was loaded fleet-wide, but **never fired in any shape**: `matchBinaries In [curl,wget]` cannot match at `execve` (the calling process is the shell, not curl), and `\| sh` is a shell construct that never appears in curl's argv. Its `sizeArgIndex: 2` also pointed at `envp`. Verified: real `curl\|sh` → curl untouched; literal `"\| sh"` argv → untouched; zero events. **Removed**, since the engine already scores the pattern correctly from the exec chain (`/bin/bash → /usr/bin/curl`, score 26, high) — which works precisely because it sees the parent's argv, as a kprobe selector cannot. A control that cannot fail is indistinguishable from no control while looking like coverage. | 1 |
| EN-2 | **Fleet-wide destructive action** — one command quarantines/severs many hosts (D) | A5, compromised CP | **Approvals / change-control** on destructive actions; **staged rollout rings** (canary→limited→fleet, reusing `production-rollout/`); a **fleet kill-switch** to halt enforcement instantly. **Partially shipped:** dual-control approvals exist in `engine/internal/approval/`, wired into the choke routes in `engine/internal/controlplane/choke.go` and enabled with `-require-approval` on the control plane. It is **opt-in, not the default**, and staged rollout rings are not built. | 2 |
| EN-3 | **Two enforcement authorities diverge** — engine detect-only but Tetragon still Sigkills (D,R) | operator confusion | Model host enforcement as *engine mode ∪ active Tetragon enforce policies*; surface both in agent self-report/health; central policy management owns both; document the coupling. **Enacted, three layers deep.** (1) *Prevented in-kernel:* every shipped policy declares `policy-mode: monitor`, under which Tetragon suppresses enforcing actions itself — measured on v1.6.1, a `Sigkill` policy returned exit 0 with the option and 137 without, while `Post` delivery was identical, so detection is untouched. It is declarative, so unlike `tetra tracingpolicy set-mode` it survives a restart. (2) *Reported:* the agent asks Tetragon (not the YAML on disk — the two disagree in both directions) for its loaded policies and carries name/mode/enabled plus the fired and suppressed action counters on the heartbeat (`DataPlaneState.kernel_policies`); the control plane exposes `kernel.diverged` on `/api/choke/state` and **names the offending agents**, since "somewhere in your fleet" is not actionable. Verified live: arming a policy on one agent flipped `diverged` to true and named it; disarming cleared it. (3) *Asserted:* [`host-posture.sh`](../../scripts/e2e/host-posture.sh) fails the suite if any host has a policy outside monitor mode, has ever fired an enforcing action, or stages an enforcing policy on disk without the monitor guard — verified against a planted policy, not merely observed passing. Host posture is now `engine mode ∪ kernel policy mode`, and an agent reporting nothing is distinguished from a clean one. | 1,2 |
