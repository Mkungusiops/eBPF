...
type ChokeSummary struct {
	...
	// True when this containment has an auto-revert armed and will thaw on its
	// own. Carried because without it a fleet operator cannot distinguish a
	// process that unfreezes in thirty minutes from one contained permanently —
	// the agent-local console has shown this since the revert timer existed, and
	// the multi-tenant console rendered the two identically.
	RevertPending bool `protobuf:"varint,6,opt,name=revert_pending,json=revertPending,proto3" json:"revert_pending,omitempty"`
	// Last time the agent observed this process. A contained exec_id whose
	// process has since exited stays in the snapshot, and without this the row
	// reads as a live containment.
...
func (x *ChokeSummary) GetRevertPending() bool {
	if x != nil {
		return x.RevertPending
	}
	return false
}

func (x *ChokeSummary) GetLastSeen() *timestamppb.Timestamp {
	if x != nil {
		return x.LastSeen
	}
	return nil
...
const file_ebpfsoc_v1_heartbeat_proto_rawDesc = "" +
	...
	"\aexec_id\x18\t \x01(\tR\x06execId\"\xdd\x01\n" +
	...
	"\x0erevert_pending\x18\x06 \x01(\bR\rrevertPending\x127\n" +
	"\tlast_seen\x18\a \x01(\v2\x1a.google.protobuf.TimestampR\blastSeen\"\x84\x01\n" +
...
var file_ebpfsoc_v1_heartbeat_proto_depIdxs = []int32{
	...
	9,  // 7: ebpfsoc.v1.ChokeSummary.last_seen:type_name -> google.protobuf.Timestamp
	9,  // 8: ebpfsoc.v1.HeartbeatResponse.server_time:type_name -> google.protobuf.Timestamp
	0,  // 9: ebpfsoc.v1.HeartbeatService.Heartbeat:input_type -> ebpfsoc.v1.HeartbeatRequest
	6,  // 10: ebpfsoc.v1.HeartbeatService.Heartbeat:output_type -> ebpfsoc.v1.HeartbeatResponse
