...
type Decision struct {
	...
	// The operator who took the action, empty for an automatic one.
	//
	// CHAIN-HASHED, and that is why it is not optional decoration. The local
	// store appends an actor tail to the canonical string it hashes, so a
	// decision that travels without its actor arrives where the receiver
	// recomputes a DIFFERENT canonical string and gets a different hash — and
	// reports the chain broken. Omitting this field does not lose a nice-to-have
	// attribution; it manufactures a tamper alarm on every operator action.
	Actor string `protobuf:"bytes,32,opt,name=actor,proto3" json:"actor,omitempty"`
...
const file_ebpfsoc_v1_common_proto_rawDesc = "" +
	...
	"\x06tactic\x18\b \x01(\tR\x06tactic\"\x99\x05\n" +
	"\bDecision\x12\x0e\n" +
	...
	"\tdevice_id\x18\x1f \x01(\tR\bdeviceId\x12\x14\n" +
	"\x05actor\x18  \x01(\tR\x05actor\x12\x1b\n" +
